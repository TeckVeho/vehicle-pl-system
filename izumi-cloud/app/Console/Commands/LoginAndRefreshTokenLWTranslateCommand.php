...
class LoginAndRefreshTokenLWTranslateCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'get_and_save_conf_lw_translate';
     ...
     * @var string
     */
    protected $description = 'Get and save config line works translate';
    ...
    {
        error_log('start LoginAndRefreshTokenLWTranslateCommand');
        Log::info('start LoginAndRefreshTokenLWTranslateCommand');
    ...
    private function checkTokenLw($systemLwAccessToken)
    {
        ...
        if ($systemLwAccessToken && data_get($systemLwAccessToken, 'value.access_token')) {
            ...
            if ($check && data_get($check, 'bots')) {
                ...
                error_log('checkTokenLwTranslate function:' . json_encode($check));
                Log::info('checkTokenLwTranslate function:' . json_encode($check));
            }
        }
    ...
    private function refreshTokenLw(&$systemLwAccessToken)
    {
        ...
        if ($systemLwAccessToken && data_get($systemLwAccessToken, 'value.access_token')) {
            ...
            if ($dataJson && data_get($dataJson, 'access_token')) {
                ...
                error_log('refreshTokenLwTranslate function:' . json_encode($dataJson));
                Log::info('refreshTokenLwTranslate function:' . json_encode($dataJson));
                $this->getAndSaveAccessTokenLw($lw_conf, $systemLwAccessToken);
            }
        }
    }
