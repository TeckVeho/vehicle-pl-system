...
class LWSendMsEvent implements ShouldBroadcastNow
{
    ...
    {
        call_user_func_array([$this, 'sendMsgVehicle'], []);
        return [App::environment() . '_vehicle_lw_channel'];
    }
    private function sendMsgVehicle()
    {
        //send when no data
        Log::info('Sending LWMS Vehicle ');
        $msg = $this->vehicle ." の車検証が連携されました。\n";
        $this->sendMsgToLW($msg);
    }
    public function broadcastAs()
    {
        return 'vehicle_lw_event';
    }
    ...
    private function sendMsgToLW($msg, $url = null)
    {
        ...
        if ($systemLwAccessToken && data_get($systemLwAccessToken, 'value.access_token')) {
            ...
            $urlApi = Str::replaceArray('?',
                [
                    Arr::get($lw_conf, 'bot_id'),
                    Arr::get($lw_conf, 'channel_id')
                ], LW_API_SEND_MSG);
