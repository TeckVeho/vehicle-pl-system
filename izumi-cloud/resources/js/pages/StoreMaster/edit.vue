<template>
	...
	>
		...
		<div class="store-master-edit">
			<div class="store-master-edit__header">
				<vHeaderPage>
			...
			<vDropdownView :dropdown-title="list_dropdown_title">
				<template #dropdown-content>
					<b-row align-h="center">
						<b-col cols="12" sm="12" md="12" lg="12" xl="12">
							...
							<b-form-input id="input-store-name" v-model="store_name" />
						</b-col>
						...
						<b-col cols="12" sm="12" md="6" lg="12" xl="12" class="mt-3">
							...
							<b-form-input id="input-display-pass-code" v-model="pass_code" type="number" :disabled="overlay.show" @keydown.native="onlyNumberInput" />
						</b-col>
					</b-row>
				</template>
				...
				<template #dropdown-content-2>
						...
						<b-col cols="12" sm="12" md="12" lg="12" xl="12">
							...
							<div v-if="delivery_manual.length" class="d-flex flex-column mt-3" style="border: 1px solid #dddddd; padding: 10px;">
								<div
									...
									:key="`delivery-${index}`"
									class="d-flex flex-row w-100 align-content-center justify-content-between"
								>
									<span>{{ item }}</span>
					...
					<b-row align-h="center" class="mt-3">
						<b-col cols="12" sm="12" md="12" lg="12" xl="12">
							<b-form-file ref="deliveryRoute" accept=".jpg, .png, .jpeg" style="display: none;" @change="previewImage($event, 1)" />
				...
				<template #dropdown-content-3>
					...
					<b-row align-h="center" class="mt-3">
						<b-col cols="12" sm="12" md="12" lg="12" xl="12">
							<b-form-file ref="parkingPosition1" accept=".jpg, .png, .jpeg" style="display: none;" @change="previewImage($event, 2)" />
				...
				<template #dropdown-content-4>
					...
					<b-row align-h="center" class="mt-3">
						<b-col cols="12" sm="12" md="12" lg="12" xl="12">
							<b-form-file ref="parkingPosition2" accept=".jpg, .png, .jpeg" style="display: none;" @change="previewImage($event, 3)" />
...
import { getOneStore, updateStore } from '@/api/modules/storeMaster';
import { onlyNumberInput, inputPostCode } from '@/utils/onlyNumberInput';

const urlAPI = {
    get: '/store',
    post: '/update-store-for-web',
};
...
export default {
    ...
    methods: {
        async handleGetOneStore() {
            ...
            try {
                ...
                if (ID) {
                    ...
                    if (res.code === 200) {
                        this.business_classification = DATA.bussiness_classification;
                        this.store_name = DATA.store_name;
                        this.delivery_destination_code = DATA.delivery_destination_code;
                        this.delivery_destination_name_kana = DATA.destination_name_kana;
                        this.delivery_destination_name = DATA.destination_name;
                        this.post_code = DATA.post_code;
                        this.tel = DATA.tel_number;
                        this.first_address = DATA.address_1;
                        this.second_address = DATA.address_2;
                        this.pass_code = DATA.pass_code ? parseInt(DATA.pass_code) : '';
        ...
        validateData() {
            ...
            } else if (this.post_code && this.post_code.length > 8) {
                this.$toast.warning({
                    content: '郵便番号は半角英数字8文字以内で入力してください。',
                });
            } else if (this.first_address && this.first_address.length > 20) {
                this.$toast.warning({
                    content: '大住所は20文字以内で入力してください。',
                });
            } else if (this.second_address && this.second_address.length > 50) {
                this.$toast.warning({
                    content: '小住所は50文字以内で入力してください。',
                });
            } else if (this.tel && this.tel.length > 13) {
                this.$toast.warning({
        ...
        async handleUpdateStore() {
            try {
                ...
                if (ID) {
                    const res = await updateStore(`${URL}/${ID}`, FORM_DATA);

                    if (res.code === 200) {
                        this.$toast.success({ content: '編集が完了しました' });
                        this.$router.push({ name: 'StoreMaster' });
                    }
                }
