		...
		<div class="main-content">
			...
			<vDropdownView :dropdown-title="listDropdownTitle">
				<template #dropdown-content>
					<b-row>
						<b-col cols="12" class="mt-3">
							...
							<vInput v-model="formData.vehicle_identification_number" :type="'text'" placeholder="入力してください" :class-name="'vehicle_identification_number'" />
						</b-col>
						...
						<b-col cols="12" class="mt-3">
							<b-row>
								<b-col cols="6">
									...
									<vSelect v-model="formData.box_distinction" :type="'text'" :data-options="listBoxDistinction" :class-name="'box_distinction'" />
								</b-col>
						...
						<b-col cols="12" class="mt-3">
							<b-row>
								<b-col cols="6">
									...
									<b-form-checkbox id="early-registration-checkbox" v-model="formData.early_registration" size="lg" :value="true" :unchecked-value="false" />
								</b-col>
						...
						<b-col cols="12" class="mt-3">
							<b-row>
								...
								<b-col cols="6">
									...
									<vInput v-model="formData.monthly_mileage" :type="'text'" placeholder="入力してください" :class-name="'monthly_mileage'" />
								</b-col>
							</b-row>
						</b-col>
				...
				<template #dropdown-content-3>
					<b-row>
						<b-col cols="12">
							<VI18NLabel :text-label="'VEHICLE_MASTER.REMARK.OLD_CAR_1'" />
							<vInput v-model="formData.old_car_1" :type="'text'" placeholder="入力してください" :class-name="'old_car_1'" />
						</b-col>
					</b-row>
				</template>
					...
					<b-row>
						<b-col cols="12" class="mt-3">
							...
							<div id="preview" class="mt-3">
								<img :src="sampleImgUrl || defaultImageUrl">
							</div>
						</b-col>

						<b-col cols="12" class="mt-3">
							<VI18NLabel :text-label="'VEHICLE_MASTER.VEHICLE_CERTIFICATION.REMARK'" />
							<vInput v-model="formData.verify_certificate_remark" :type="'text'" placeholder="" disabled :class-name="'verify_certificate_remark'" />
						</b-col>
					</b-row>
				</template>
			</vDropdownView>
			...
			<b-row class="footer">
				...
				<b-col cols="6" class="p-0 text-right">
					<b-button class="functional-button btn-save-main" @click="onClickRegisterButton()">登録</b-button>
				</b-col>
			</b-row>
		</div>
	</b-overlay>
...
import { createVehicle, getListDepartment } from '@/api/modules/vehicleMaster';

import { handleGetCertificateByVehicleTotalWeight } from './helper/helper';
import { validInputNumber, validInputHalfWidthNumber, validInputHalfWidthNumberPaste } from '@/utils/handleInput';

const urlAPI = {
    apiCreateVehicle: '/vehicle',
    ...
    methods: {
        ...
        async onClickRegisterButton() {
            ...
            if (this.handleValidateData(this.formData)) {
                try {
                    const URL = urlAPI.apiCreateVehicle;
                    ...
                    const DATA = {
                        ...
                        early_registration: this.formData.early_registration,
                        d1d_not_installed: this.formData.d1d_not_installed,
                    ...
                    const response = await createVehicle(URL, DATA);
        ...
        handleProcessPrevious() {
            const THIRTY_DAYS = [4, 6, 9, 11];
            const THIRTY_ONE_DAYS = [1, 3, 5, 7, 8, 10, 12];

            if (this.datePicker.day > 1) {
                this.datePicker.day -= 1;
            } else {
                if (this.datePicker.month > 1) {
                    this.datePicker.month -= 1;

                    if (THIRTY_DAYS.includes(this.datePicker.month)) {
                        this.datePicker.day = 30;
                    } else if (THIRTY_ONE_DAYS.includes(this.datePicker.month)) {
                        this.datePicker.day = 31;
                    } else {
                        if (this.checkLeapYear(this.datePicker.year)) {
                            this.datePicker.day = 29;
                        } else {
                            this.datePicker.day = 28;
                        }
                    }
                } else {
                    this.datePicker.year -= 1;
                    this.datePicker.month = 12;
                    this.datePicker.day = 31;
                }
            }

            if (this.datePicker.day % 2 === 0) {
                this.formData.default_image_url = defaultImageUrl;
            } else {
                this.formData.default_image_url = sampleImgUrl;
            }
        },
        handleProcessNext() {
            const THIRTY_DAYS = [4, 6, 9, 11];
            ...
            if (THIRTY_DAYS.includes(this.datePicker.month)) {
                if (this.datePicker.day < 30) {
                    this.datePicker.day += 1;
                } else {
                    if (this.datePicker.month < 12) {
                        this.datePicker.month += 1;
                        this.datePicker.day = 1;
                    } else {
                        this.datePicker.year += 1;
        ...
        handleValidateData(DATA) {
            ...
            if (DATA.vehicle_identification_number === '') {
                MakeToast({
                    variant: 'warning',
                    title: this.$t('WARNING'),
                    content: '車体番号を入力してください',
                });
            } else if (DATA.no_number_plate === '') {
                MakeToast({
                    variant: 'warning',
                    title: this.$t('WARNING'),
            ...
            } else if (DATA.vehicle_delivery_date === '') {
                MakeToast({
                    variant: 'warning',
                    title: this.$t('WARNING'),
                    content: '車両納車日が必要です',
