									...
									<b-tr>
										...
										<b-td class="td-file">
											...
											<div v-if="table.list_movie.front.progressPercentage === 100" class="dropzone-custom-content extra" style="margin-top: 5px;">
												<div :class="['w-100 text-center completed-bar', table.list_movie.front.progressPercentage === 100 ? 'status-success' : status]">
									...
									<b-tr>
										...
										<b-td class="td-file">
											...
											<div v-if="table.list_movie.inside.progressPercentage === 100" class="dropzone-custom-content extra" style="margin-top: 5px;">
												<div :class="['w-100 text-center completed-bar', table.list_movie.inside.progressPercentage === 100 ? 'status-success' : status]">
													<div class="d-flex flex-row w-100 justify-content-between pl-2 pr-2" style="position: relative;">
														<p class="completed-bar-file-name" style="padding: 0px !important;">{{ `${table.list_movie.inside.fileName}` }}</p>
														<i class="far fa-times-circle completed-bar-icon" @click="handleRemoveVideo(table.id, 'inside', table.list_movie.inside.id)" />
													</div>
												</div>
											</div>
									...
									<b-tr>
										...
										<b-td class="td-file">
											...
											<div v-if="table.list_movie.behind.progressPercentage === 100" class="dropzone-custom-content extra" style="margin-top: 5px;">
												<div :class="['w-100 text-center completed-bar', table.list_movie.behind.progressPercentage === 100 ? 'status-success' : status]">
													<div class="d-flex flex-row w-100 justify-content-between pl-2 pr-2" style="position: relative;">
						...
						<div v-for="(table, idx) in sampleUploadData" :key="idx" class="table-view-upload-file">
							<div class="title-movie">
								...
								<span>
									...
									<b-form-input id="movie-title-input" v-model="table.movie_title" />
								</span>
							</div>
...
export default {
    ...
    methods: {
        ...
        async afterComplete(response, id, position) {
