		...
		<div class="driver-recorder-detail">
			...
			<div class="driver-recorder-detail__form">
				<div class="item-form">
					...
					<b-form-input id="form-record-date" v-model="isForm.record_date" disabled />
				</div>

				<div class="item-form">
					<label for="form-department">{{ $t('DRIVER_RECORDER.LABEL_DEPARTMENT') }}</label>
					<b-form-input id="form-record-date" v-model="isForm.department" disabled />
				</div>

				<!-- 2707 -->
				<div class="item-form">
					<label for="type-one">事故GP</label>
					<b-form-radio-group id="type-one" v-model="isForm.type_one" :options="type_one_options" disabled />
				</div>

				<div class="item-form">
					<label for="type-two">有責無責</label>
					<b-form-radio-group id="type-two" v-model="isForm.type_two" disabled>
						<b-form-radio value="1" class="custom-radio-option">
							<span v-if="isForm.type_two === 1" style="color: red;">有責</span>
							<span v-else>有責</span>
						</b-form-radio>
				...
				<div class="item-form">
					...
					<b-form-radio-group id="shipper" v-model="isForm.shipper" :options="shipper_options" disabled />
				</div>

				<div class="item-form">
					<label for="accident-classification">事故区分</label>
					<b-form-radio-group id="accident-classification" v-model="isForm.accident_classification" :options="accident_classification_options" disabled />
				</div>

				<div class="item-form">
					<label for="place-of-occurrence">発生場所</label>
					<b-form-radio-group id="place-of-occurrence" v-model="isForm.place_of_occurrence" :options="place_of_occurrence_options" disabled />
				</div>

				<div class="item-form">
					<label for="form-title">{{ $t('DRIVER_RECORDER.LABEL_TITLE') }}</label>
					<b-form-input id="form-title" v-model="isForm.title" disabled />
				</div>

				<div class="item-form">
					<label for="">{{ $t('DRIVER_RECORDER.LABEL_UPLOAD_FILE') }}</label>
					<UploadFile :is-edit="false" :sample-upload-data="isForm.listUploadFile" />
				</div>
				...
				<div class="d-flex flex-row w-100 mt-3">
					<div class="d-flex w-50 justify-content-start">
						<vButton :text-button="$t('BUTTON.BACK')" :class-name="'btn-radius-default v-button-default btn-registration'" @click.native="onClickBack()" />
					</div>

					<div class="d-flex flex-row w-50 justify-content-end align-items-center">
						<b-form-checkbox v-model="isForm.is_create_with_notice" class="mr-3" value="accepted" unchecked-value="not_accepted" size="lg" disabled>
							<span style="color: red;">お知らせを自動作成する</span>
						</b-form-checkbox>

						<template v-if="hasAccessEdit.includes(role)">
							<vButton :text-button="$t('BUTTON.EDIT')" :class-name="'btn-radius-default v-button-default btn-registration'" @click.native="onClickEdit()" />
						</template>
					</div>
				</div>
...
import { getDetailFile, getListDepartment } from '@/api/modules/driverRecorder';
...
export default {
    ...
    data() {
        return {
            ...
            listRecordType: [{
                value: 0,
                text: this.$t('DRIVER_RECORDER_INDEX.ACCIDENT'),
            },
            {
                value: 1,
                text: this.$t('DRIVER_RECORDER_INDEX.OTHER'),
            },
            ],
    ...
    methods: {
        ...
        async handleGetDetailFile() {
            try {
                ...
                if (response.code === 200) {
                    ...
                    this.isForm.crew_member = this.convertCrewInDepartment(this.items.crew_member_id);
        ...
        async handleGetListDepartment() {
            try {
                const {
                    code,
                    data,
                } = await getListDepartment(urlAPI.apiGetListDepartment);
        ...
        convertCrewInDepartment(user_id) {
            const crew = this.listCrewMemberByDepartment.find((item) => item.id === user_id);
