...
<template>
    <b-overlay :show="overlay.show" :variant="overlay.variant" :opacity="overlay.opacity" :blur="overlay.blur" :rounded="overlay.sm">
        ...
        <div class="driver-recorder-create">
            <div class="driver-recorder-create__title-header">
                <vHeaderPage>
                    {{ $t('PAGE_TITLE.DRIVER_RECONRDER_LIST') }}
                </vHeaderPage>
            </div>

            <div class="driver-recorder-create__form">
                <div class="item-form">
                ...
                <div class="item-form">
                    ...
                    <b-form-select id="form-department" v-model="isForm.department" @change="handleGetListUserByDepartment()">
                        <b-form-select-option :value="null" :disabled="true">{{ $t('PLEASE_SELECT') }}</b-form-select-option>
                        <b-form-select-option v-for="(option, index) in listDeparment" :key="index" :value="option.value" :disabled="option.value === null">
                            {{ option.text }}
                ...
                <div class="item-form">
                    ...
                    <b-form-radio-group id="type-two" v-model="isForm.type_two" :options="type_two_options" />
                </div>
                ...
                <div class="item-form">
                    ...
                    <UploadFile :is-edit="true" :sample-upload-data="sampleUploadData" @add="eventAddTableUploadFile" @delete="eventDeleteTableUploadFile" />
                </div>
                    ...
                    <div class="d-flex-row justify-content-between">
                        <div class="card-holder">
                            <template v-if="attached_image_1_path && !attached_image_1_is_pdf">
                                <img :src="attached_image_1_path" alt="" class="image-card">
                        ...
                        <div class="card-holder">
                            <template v-if="attached_image_2_path && !attached_image_2_is_pdf">
                                <img :src="attached_image_2_path" alt="" class="image-card">
                        ...
                        <div class="card-holder">
                            <template v-if="attached_image_3_path && !attached_image_3_is_pdf">
                                <img :src="attached_image_3_path" alt="" class="image-card">
                        ...
                        <div class="card-holder">
                            <template v-if="attached_image_4_path && !attached_image_4_is_pdf">
                                <img :src="attached_image_4_path" alt="" class="image-card">
                        ...
                        <div class="card-holder">
                            <template v-if="attached_image_5_path && !attached_image_5_is_pdf">
                                <img :src="attached_image_5_path" alt="" class="image-card">
                ...
                <div class="d-flex-row justify-content-between mt-3">
                    <div class="card-holder">
                        <template v-if="attached_image_6_path && !attached_image_6_is_pdf">
                            <img :src="attached_image_6_path" alt="" class="image-card">
                    ...
                    <div class="card-holder">
                        <template v-if="attached_image_7_path && !attached_image_7_is_pdf">
                            <img :src="attached_image_7_path" alt="" class="image-card">
                    ...
                    <div class="card-holder">
                        <template v-if="attached_image_8_path && !attached_image_8_is_pdf">
                            <img :src="attached_image_8_path" alt="" class="image-card">
                    ...
                    <div class="card-holder">
                        <template v-if="attached_image_9_path && !attached_image_9_is_pdf">
                            <img :src="attached_image_9_path" alt="" class="image-card">
                    ...
                    <div class="card-holder">
                        <template v-if="attached_image_10_path && !attached_image_10_is_pdf">
                            <img :src="attached_image_10_path" alt="" class="image-card">
                ...
                <div class="item-form mt-3">
                    ...
                    <template v-if="attached_file_path">
                        <div class="file-preview-card">
                            <span>
                                <span class="text-preview" @click="handlePreviewFile(6)">{{ `${attached_file_name}
                ...
                <div class="d-flex flex-row w-100 mt-3">
                    ...
                    <div class="d-flex flex-row w-50 justify-content-end align-items-center">
                        <b-form-checkbox v-model="is_create_with_notice" class="mr-3" value="accepted" unchecked-value="not_accepted" size="lg">
                            <span style="color: red;">お知らせを自動作成する</span>
                        </b-form-checkbox>

                        <vButton :text-button="$t('BUTTON.UPLOAD')" :class-name="'btn-radius-default v-button-default btn-registration'" :disabled="hasProcessRunning" @click.native="onClickUpload()" />
                    </div>
                </div>
            </div>
        </div>
...
import { postUploadData, getListDepartment } from '@/api/modules/driverRecorder';
...
export default {
    ...
    methods: {
        ...
        async handleGetListDepartment() {
            ...
            try {
                ...
                if (response.code === 200) {
                    ...
                    this.listDeparment = JSON.parse(JSON.stringify(DATA));
                }
            } catch (error) {
                console.log(error);
            }
        ...
        onClickBack() {
            this.$router.push({ name: 'DriverRecorderList' });
        },
            ...
            if (this.handleValidateFormData()) {
                try {
                    ...
                    const DATA = {
                        ...
                        is_draft: this.is_create_with_notice === 'accepted' ? 1 : 0,
                        flag_send_noti: this.is_create_with_notice === 'accepted' ? 1 : 0,
                    };
                    ...
                    const response = await postUploadData(URL, DATA);

                    if (response.code === 200) {
                        MakeToast({
                            variant: 'success',
                            title: this.$t('SUCCESS'),
                            content: '新規アップロードに成功しました',
                        });

                        if (this.is_create_with_notice === 'accepted') {
                            await this.handleCreateNotification(response.data?.id);
                        }
        ...
        handleProcessRemoveUploadData(id, position) {
            ...
            for (let i = 0; i < this.isForm.listUploadFile.length; i++) {
                if (id === this.isForm.listUploadFile[i].id) {
                    this.isForm.listUploadFile[i].list_movie[position] = '';
                }
            }
        },
        ...
        async handleCreateNotification(driver_recorder_id) {
            try {
                ...
                const departmentName = this.listDeparment.find(item => item.value === this.isForm.department);
                const title = `【原因対策】${this.handleConvertTime(this.isForm.record_date)} - ${departmentName?.text} - ${this.isForm.title}`;
...
::v-deep .vue-dropzone>.dz-preview .dz-details {
    background-color: rgb(177 194 207 / 80%) !important;
}

.driver-recorder-create {
    overflow: hidden;
