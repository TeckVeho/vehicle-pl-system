...
**タイムシート**からドライバー乗務記録・給与データを連携し、乗務員給料・通勤手当・法定福利費などを車両別に按分する機能の API 仕様です。（業務給料・福利厚生費は手動インポート）ドライバーマスタは**イズミクラウド**から連携し、ドライバーとコースの紐づきは**ATMTC**で取得したものをイズミクラウド経由で連携します。
...
| POST | /api/daily-operating/sync | 日次稼働・走行データ一括同期（認証必須・MASTER）。連携後にドライバー配賦（乗車回数ベース等）を再実行 |

※ いずれも認証必須。sync 系は DX/DX管理者（MASTER）権限が必要。
...
- 勘定科目は `isDriverRelated: true` のもののみ対象（乗務員給料、通勤手当、法定福利費など。業務給料・福利厚生費は手動インポートのため対象外）
- 連携後に配賦計算を自動実行
- レスポンスに `allocation`（配賦結果サマリー）を含む
...
※業務給料 (6139)・福利厚生費 (6149) は手動インポートのため `isDriverRelated: false`

**乗務員給料・通勤手当の配賦ロジック**: 乗車回数ベースで配賦。各ドライバーの1日単価×乗車回数を車両別に累計し MonthlyRecord に保存。driver-assignments / driver-monthly-amounts / daily-operating の sync 時に実行。詳細は [account-item-calculation-spec.md](account-item-calculation-spec.md) 参照。
