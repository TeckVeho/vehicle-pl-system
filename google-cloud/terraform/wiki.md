...
Chọn Tier dựa trên đặc điểm dịch vụ. Tier được ghi trong `environments/{env}/app (and network/)` và áp dụng cho toàn bộ môi trường prod / stg / dev.
...
| Quy tắc                 | Nội dung                                                                                     |
| ----------------------- | -------------------------------------------------------------------------------------------- |
| **Nâng Tier**           | Khi lưu lượng tăng hoặc yêu cầu SLA cao hơn → PR vào `google-cloud/terraform/` + review                       |
| **Hạ Tier**             | Mục đích giảm chi phí → PR vào `google-cloud/terraform/` + review bởi SRE                                     |
...
```hcl
# environments/{env}/app (and network/)
...
├── src/                            # Code ứng dụng
├── Dockerfile
├── google-cloud/terraform/                          # Infrastructure (Terraform)
│   └── environments/
│       ├── dev/
│       │   ├── backend.tf
...
└── .github/
    └── workflows/
        ...
        ├── terraform-plan.yml      # Kiểm tra infra (chỉ chạy khi google-cloud/terraform/ thay đổi)
        └── terraform-apply.yml     # Áp dụng infra (sau khi merge google-cloud/terraform/ vào main)
```
...
サービスの特性に応じて Tier を選択します。Tier は `environments/{env}/app (and network/)` に記載し、prod / stg / dev 環境の全リソースに適用されます。
...
| ルール              | 内容                                                           |
| ---------------- | ------------------------------------------------------------ |
| **Tier アップ**     | トラフィック増加・SLA 要件上昇 → `google-cloud/terraform/` への PR + レビュー                    |
| **Tier ダウン**     | コスト削減目的 → `google-cloud/terraform/` への PR + SRE レビュー必須                         |
...
├── src/                            # アプリケーションコード
├── Dockerfile
├── google-cloud/terraform/                          # インフラ（Terraform）
│   └── environments/
│       ├── dev/
...
└── .github/
    └── workflows/
        ...
        ├── terraform-plan.yml      # インフラ plan（google-cloud/terraform/ 変更時のみ実行）
        └── terraform-apply.yml     # インフラ apply（google-cloud/terraform/ の main マージ後）
```
...
on:
  pull_request:
    paths:
      - "google-cloud/terraform/**"           # google-cloud/terraform/ フォルダの変更時のみ実行
...
jobs:
  plan:
    ...
    steps:
      ...
      - name: Terraform Init
        run: terraform init
        working-directory: google-cloud/terraform/environments/dev/app
...
**③ `terraform-apply.yml`（インフラ適用 / `google-cloud/terraform/` の main マージ後）**
...
jobs:
  apply:
    ...
    steps:
      ...
      - name: Terraform Init
        run: terraform init
        working-directory: google-cloud/terraform/environments/prod
...
```
google-cloud/terraform/ を変更して PR を提出
  ↓
...
| `terraform-plan.yml` | `pull_request` | `google-cloud/terraform/**` | terraform plan → PR コメント |
| `terraform-apply.yml` | `push` to `main` | `google-cloud/terraform/**` | terraform apply（prod は承認必須） |
